...
public sealed class Design : DesignBase, ISavable, IDesignStandIn, IFileSystemValue<Design>, JsonObjectConversion.IJsonWritable<Design>
{
        ...
        : base(other)
    {
        ...
        CustomizePlusApplicationMode  = other.CustomizePlusApplicationMode;
        Color                         = other.Color;
    ...
    /// <summary>
    /// Written as upstream <c>FileVersion</c> 3. Customize+ fields are extra JSON keys that stock Glamourer ignores.
    /// When merging upstream: if they introduce a new <c>FileVersion</c>, extend <see cref="LoadDesign"/> without reusing that number for fork-only semantics.
    /// </summary>
    ...
    public new void Serialize(Utf8JsonWriter j)
    {
        ...
        j.WriteIfNot("ApplyCustomizePlusAssociation"u8, ApplyCustomizePlusAssociation, true);
        if (CustomizePlusApplicationMode is not CustomizePlusApplicationMode.PermanentProfile)
            j.WriteString("CustomizePlusApplicationMode"u8, CustomizePlusApplicationMode.ToString());
        ...
        SerializeMods(j);
        CustomizePlusAssociation.WriteJson(j, "CustomizePlusAssociation"u8);
        ...
        int version)
    {
        ...
        design.ApplyCustomizePlusAssociation = json.PropertyOrDefault("ApplyCustomizePlusAssociation"u8, true);
        design.CustomizePlusApplicationMode  = json.EnumOrDefault("CustomizePlusApplicationMode"u8,
            CustomizePlusApplicationMode.PermanentProfile);
        design.CustomizePlusAssociation      = CustomizePlusAssociation.Load(
            json.TryReadObject("CustomizePlusAssociation"u8, out var cPlus) ? cPlus : null);
        return design;
    ...
    private static void LoadMods(in JsonElement? mods, Design design, int version)
    {
        ...
        foreach (var mod in mods.Value.EnumerateArray())
        {
            ...
            {
                Glamourer.Messager.NotificationMessage("加载的设计包含无效模组，已跳过。", NotificationType.Warning);
                continue;
            }
            ...
            if (!design.AssociatedMods.TryAdd(new ModIdentifier(directory!, name!), preset))
                Glamourer.Messager.NotificationMessage("加载的设计包含重复模组，已跳过。", NotificationType.Warning);
        }
