...
public partial class CustomizationDrawer
{
    ...
    private void DrawRaceGenderSelector()
    {
        ...
        if (_withApply)
        {
            ...
            if (UiHelpers.DrawCheckbox("##applyGender"u8, "应用此设计的性别。"u8, ChangeApply.HasFlag(CustomizeFlag.Gender),
                    out var applyGender, _locked))
                ChangeApply = applyGender ? ChangeApply | CustomizeFlag.Gender : ChangeApply & ~CustomizeFlag.Gender;
            Im.Line.Same();
            if (UiHelpers.DrawCheckbox("##applyClan"u8, "应用此设计的部族。"u8, ChangeApply.HasFlag(CustomizeFlag.Clan), out var applyClan,
                    _locked))
                ChangeApply = applyClan ? ChangeApply | CustomizeFlag.Clan : ChangeApply & ~CustomizeFlag.Clan;
            Im.Line.Same();
        }

        ImEx.TextFrameAligned("性别 & 部族"u8);
    }
    ...
    private void DrawBodyType()
    {
        ...
        if (!ImEx.Button(_lockedRedraw
                    ? $"体型 {_customize.BodyType.Value}"
                    : $"重置体型 {_customize.BodyType.Value} 为默认",
