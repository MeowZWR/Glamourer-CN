...
public partial class CustomizationDrawer
{
    ...
    private void DrawHeight()
    {
        ...
        if (config.HeightDisplayType switch
            {
                HeightDisplayType.Centimetre => Utf8.TryWrite(t, ic, $"({height * 100:F1} 厘米)",         out written),
                HeightDisplayType.Metre      => Utf8.TryWrite(t, ic, $"({height:F2} 米)",                out written),
                HeightDisplayType.Wrong      => Utf8.TryWrite(t, ic, $"({height * 100 / 2.539:F1} 英寸)", out written),
                HeightDisplayType.WrongFoot => Utf8.TryWrite(t, ic, $"({(int)(height * 3.2821)}'{(int)(height * 39.3856) % 12}'')",
                    out written),
                HeightDisplayType.Corgi       => Utf8.TryWrite(t, ic, $"({height * 100 / 40.0:F1} 柯基)", out written),
                HeightDisplayType.OlympicPool => Utf8.TryWrite(t, ic, $"({height / 3.0:F3} 泳池)",         out written),
    ...
    private void PercentageInputInt()
    {
        ...
        Im.Tooltip.OnHover($"输入范围：[0, {_currentCount - 1}]\n"
    ...
    private void DataInputInt(int currentIndex, bool npc)
    {
        ...
        if (!_withApply)
            Im.Tooltip.OnHover("按住Ctrl键强制输入无效/未知参数，风险自负。");

        var size = new Vector2(Im.Style.FrameHeight);
        Im.Line.Same();
        if (ImEx.Button("-"u8, size, "按顺序选择上一个可用选项。"u8, currentIndex <= 0))
            UpdateValue(_set.Data(_currentIndex, currentIndex - 1, _customize.Face).Value);
        else
            CheckWheel();
        Im.Line.Same();
        if (ImEx.Button("+"u8, size, "按顺序选择下一个可用选项。"u8, currentIndex >= _currentCount - 1 || npc))
            UpdateValue(_set.Data(_currentIndex, currentIndex + 1, _customize.Face).Value);
        else
            CheckWheel();
        return;
    ...
    private void ListInputInt0()
    {
        ...
        Im.Tooltip.OnHover($"输入范围：[1, {_currentCount}]\n"
          + "按住Ctrl键强制输入无效/未知参数，风险自负。");
    }
