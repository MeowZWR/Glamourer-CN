...
public sealed class EquipmentDrawer : IUiService, IDisposable
{
    ...
    public bool DrawAllStain(out StainIds ret, bool locked)
    {
        ...
        var       change   = _stainCombo.Draw("为所有装备槽染色"u8, Stain.None, out var newAllStain, Im.Style.FrameHeight);
        ...
        if (!locked)
        {
            ...
            Im.Tooltip.OnHover($"{_config.DeleteDesignModifier.ToString()} + 并单击右键进行清理。");
        }
        ...
        (var tt, item, var valid) = (allowRevert && !revertItem.Equals(currentItem), allowClear && !clearItem.Equals(currentItem),
                Im.Io.KeyControl) switch
            {
                (true, true, true) => RefTuple.Create(
                    "右键单击进行清除。Ctrl+右键单击可恢复到游戏状态。\nCtrl+鼠标滚轮滚动。"u8,
                    revertItem, true),
                (true, true, false) => RefTuple.Create(
                    "右键单击进行清除。Ctrl+右键单击可恢复到游戏状态。\nCtrl+鼠标滚轮滚动。"u8,
                    clearItem, true),
                (true, false, true) => RefTuple.Create("Ctrl+右键单击可恢复到游戏状态。\nCtrl+鼠标滚轮滚动。"u8,
                    revertItem, true),
                (true, false, false) => RefTuple.Create("Ctrl+右键单击可恢复到游戏状态。\nCtrl+鼠标滚轮滚动。"u8,
                    (T?)default!, false),
                (false, true, _)  => RefTuple.Create("右键单击进行清除。\nCtrl+鼠标滚轮滚动。"u8, clearItem,    true),
                (false, false, _) => RefTuple.Create("Ctrl+鼠标滚轮滚动。"u8,                        (T?)default!, false),
            };
    ...
    private static void DrawApply(in EquipDrawData data)
    {
        ...
        if (UiHelpers.DrawCheckbox("##apply"u8, "当应用此设计时也应用此项。"u8, data.CurrentApply, out var enabled,
                data.Locked))
    ...
    {
        using var id = Im.Id.Push((int)data.Slot);
        if (UiHelpers.DrawCheckbox("##apply"u8, "应用设计时应用此额外项。"u8, data.CurrentApply, out var enabled,
                data.Locked))
            data.SetApplyItem(enabled);
    }
    ...
    {
        using var id = Im.Id.Push((int)data.Slot);
        if (UiHelpers.DrawCheckbox("##applyStain"u8, "应用设计时应用染色到此项。"u8, data.CurrentApplyStain,
                out var enabled,
                data.Locked))
    ...
    private void WeaponHelpMarker(bool hasAdvancedDyes, StringU8 label, StringU8? type = null)
    {
        ...
        LunaStyle.DrawAlignedHelpMarker(
            "将武器更换为不同类型的武器可能会导致碰撞、冻结、软锁和硬锁以及作弊，"u8
          + "因此，只允许将武器改为其他同类武器。"u8);
    ...
    private void DrawEquipLabel(bool hasAdvancedDyes, StringU8 label)
    {
        ...
        if (hasAdvancedDyes)
            Im.Tooltip.OnHover("此设计拥有该槽的高级染色设置。"u8);
    }

    public static void DrawKeepItemFilter(Configuration config)
    {
        if (Im.Checkbox("选择后保留物品和染色筛选"u8, config.KeepItemComboFilter))
            config.KeepItemComboFilter ^= true;
        Im.Tooltip.OnHover(
            "选择后是否保留物品和染色筛选。\n\n这也可以用于限制鼠标滚轮滚动到匹配的物品。"u8);
    }
