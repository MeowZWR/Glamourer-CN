    ...
    TextureArraySlicePickers textureArraySlicePickers) : IService
{
    ...
    public void DrawButton(MaterialValueIndex index, Rgba32 color, bool sameLine, bool hasDyes)
    {
        ...
        if (hasDyes)
        {
            if (Im.Item.Hovered())
            {
                ...
                Im.Text("打开此槽的高级染色窗口。"u8);
                Im.Text($"\n按住 {LunaStyle.Modifier.Misclick} 并右键点击以移除此槽的所有高级染色。");
            }
        ...
        {
            Im.Tooltip.OnHover("打开此槽的高级染色窗口。"u8);
        }
    }
    ...
    private void DrawToggles()
    {
        ...
        if (_mode is ColorRow.Mode.Dawntrail)
        {
            ...
            if (ImEx.ButtonCorners("基础"u8, buttonSize, ButtonFlags.MouseButtonLeft, Corners.Left))
            {
                _editSheen = false;
                _editExtra = false;
            }
            ...
            if (ImEx.ButtonCorners("光泽"u8, buttonSize, ButtonFlags.MouseButtonLeft, Corners.None))
            {
                _editSheen = true;
                _editExtra = false;
            }
            ...
            if (ImEx.ButtonCorners("额外"u8, buttonSize, ButtonFlags.MouseButtonLeft, Corners.Right))
            {
                _editSheen = false;
                _editExtra = true;
            }
        ...
        ReadOnlySpan<FFXIVClientStructs.Interop.Pointer<MaterialResourceHandle>> materials, bool centered)
    {
        ...
        using var window = Im.Window.Begin("###Glamourer 高级染色"u8, flags);
        if (Im.Window.Appearing || _forceFocus)
        {
            Im.Window.SetFocus("###Glamourer 高级染色"u8);
            _forceFocus = false;
        }
    ...
    private void DrawAllRow(MaterialValueIndex materialIndex, in ColorTable.Table table)
    {
        ...
        ImEx.Icon.Button(LunaStyle.OnHoverIcon, "高亮显示角色身上所有受影响的颜色。"u8);
        if (Im.Item.Hovered())
        ...
        if (ImEx.Icon.Button(LunaStyle.ToClipboardIcon, "将此颜色集表导出到剪贴板。"u8))
        {
        ...
        Im.Line.SameInner();
        if (ImEx.Icon.Button(LunaStyle.FromClipboardIcon, "将导出的表从剪贴板导入到此表。"u8)
        ...
        Im.Line.SameInner();
        if (ImEx.Icon.Button(LunaStyle.UndoIcon, "将此表重置为游戏状态。"u8, !_anyChanged))
    ...
    private void DrawRow(ref ColorTableRow row, MaterialValueIndex index, in ColorTable.Table table)
    {
        ...
        ImEx.Icon.Button(LunaStyle.OnHoverIcon, "高亮显示角色身上受影响的颜色。"u8);
        if (Im.Item.Hovered())
        ...
        var applied = ImEx.ColorPickerButton("##diffuse"u8, "更改此行的漫反射值。"u8, value.Model.Diffuse,
            out value.Model.Diffuse, 'D');

        var spacing = Im.Style.ItemInnerSpacing;
        Im.Line.Same(0, spacing.X);
        applied |= ImEx.ColorPickerButton("##specular"u8, "更改此行的镜面反射值。"u8, value.Model.Specular,
            out value.Model.Specular, 'S');

        Im.Line.Same(0, spacing.X);
        applied |= ImEx.ColorPickerButton("##emissive"u8, "更改此行的发光值。"u8, value.Model.Emissive,
        ...
        else if (_mode is ColorRow.Mode.Dawntrail && _editExtra)
        {
            ...
            Im.Tooltip.OnHover("更改此行的曝光值。"u8);
            ...
            Im.Tooltip.OnHover((index.RowIndex & 1) is 0
                ? "更改此行的各向异性程度。"u8
                : "更改此行的各向异性程度。\n除非使用着色器模组，否则这对 B 行没有效果。"u8);

            Im.Line.Same(0, spacing.X);

            Im.Item.SetNextWidth(itemWidth);
            applied |= InputSphereMapIndex(textureArraySlicePickers, "更改此行的球体贴图。"u8, ref value.Model.SphereMapIndex,
                false);
            ...
            Im.Tooltip.OnHover("更改此行的球体贴图强度。"u8);
        }
        else
        {
            ...
            Im.Tooltip.OnHover(editAsRoughness ? "更改此行的粗糙度。"u8 : "更改此行的光泽强度。"u8);
            ...
            if (_mode is not ColorRow.Mode.Dawntrail)
            {
                ...
                Im.Tooltip.OnHover("更改此行的镜面反射强度。"u8);
            }
            else
            {
                Im.Item.SetNextWidthScaled(100);
                applied |= DragMetalness(ref value.Model.Metalness, false);
                Im.Tooltip.OnHover("更改此行的金属度。"u8);
            }
        }
        ...
        Im.Line.Same(0, spacing.X);
        if (ImEx.Icon.Button(LunaStyle.UndoIcon, "将此行重置为游戏状态。"u8, !changed))
