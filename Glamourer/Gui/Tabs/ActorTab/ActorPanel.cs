...
public sealed class ActorPanel : IPanel
{
    ...
    private void DrawCustomizationsHeader()
    {
        ...
        using var h = Im.Tree.HeaderId(_selection.State!.ModelData.ModelId is 0
                ? "外貌###Customization"u8
                : $"外貌（模型 ID #{_selection.State.ModelData.ModelId})###Customization",
    ...
    {
        if (ImEx.Button("恢复游戏状态"u8, Vector2.Zero, "恢复角色到游戏中的真实状态。"u8,
                _selection.State!.IsLocked))
            _stateManager.ResetState(_selection.State!, StateSource.Manual, isFinal: true);

        Im.Line.Same();

        if (ImEx.Button("重新应用自动执行"u8, Vector2.Zero,
                "基于角色当前状态，重新应用当前的自动化状态。"u8,
        ...
        Im.Line.Same();
        if (ImEx.Button("恢复到自动执行"u8, Vector2.Zero,
                "尝试将角色恢复到使用自动化设计的状态。"u8,
                !_config.EnableAutoDesigns || _selection.State!.IsLocked))
        {
        ...
        Im.Line.Same();
        if (ImEx.Button("重新应用"u8, Vector2.Zero,
                "尝试重新应用配置的状态。出现问题时使用，通常不需要。"u8,
                _selection.State!.IsLocked))
