...
public sealed class ImportService(CustomizeService customizations, IDragDropManager dragDropManager, ItemManager items) : IService
{
    public void CreateDatSource()
        ...
        {
            Im.Text($"拖拽 [{Path.GetFileName(m.Files[0])}] 导入外貌数据到 Glamourer...");
            return true;
        });
    ...
    public void CreateCharaSource()
        => dragDropManager.CreateImGuiSource("CharaDragger",
            ...
            {
                Im.Text($"拖拽 [{Path.GetFileName(m.Files[0])}] 导入 Anamnesis/CMTool 数据到 Glamourer...");
                return true;
            });
    ...
    public bool LoadChara(string path, [NotNullWhen(true)] out DesignBase? design, out string name)
    {
        ...
        {
            Glamourer.Messager.NotificationMessage(ex, $"无法读取 .chara 文件 [{path}]。", NotificationType.Error);
            design = null;
            name   = string.Empty;
            return false;
        }
    ...
    public bool LoadCma(string path, [NotNullWhen(true)] out DesignBase? design, out string name)
    {
        ...
        {
            Glamourer.Messager.NotificationMessage(ex, $"无法读取 .cma 文件 [{path}]。", NotificationType.Error);
            design = null;
            name   = string.Empty;
            return false;
        }
    ...
    public bool LoadDat(string path, out DatCharacterFile file)
    {
        ...
        {
            Glamourer.Messager.NotificationMessage(ex, $"无法读取角色数据文件 [{path}]。", NotificationType.Error);
            file = default;
        }
    ...
    public bool SaveDesignAsDat(string path, in CustomizeArray input, string description)
    {
        ...
        {
            Glamourer.Messager.NotificationMessage(ex, $"无法保存角色数据到文件 [{path}]。", "失败", NotificationType.Error);
            return false;
        }
    }
